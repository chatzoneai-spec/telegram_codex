...
from telecode.topicbot import run_polling
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Telecode server/bot runner")
    default_host = os.getenv("TELECODE_HOST", "0.0.0.0")
    default_port = int(os.getenv("TELECODE_PORT", "8000"))
    default_mode = os.getenv("TELECODE_MODE", "webhook").strip().lower() or "webhook"
    default_state_file = os.getenv(
        "TELECODE_STATE_FILE",
        os.path.join(os.getcwd(), ".telecode.state.json"),
    )
    default_projects_file = os.getenv(
        "TELECODE_PROJECTS_FILE",
        os.path.join(os.getcwd(), ".telecode.projects.json"),
    )
    ...
    parser.add_argument(
        ...
        help="Transport mode: Telegram webhook server or Telegram long polling",
    )
    parser.add_argument(
        "--reload",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="Persistent runtime state file for polling offset and topic sessions",
    )
    parser.add_argument(
        "--projects-file",
        default=default_projects_file,
        help="JSON file describing the allowed project directories",
    )
    ...
    os.environ["TELECODE_MODE"] = args.mode
    os.environ["TELECODE_STATE_FILE"] = args.state_file
    os.environ["TELECODE_PROJECTS_FILE"] = args.projects_file
    if args.verbose:
    ...
    if args.mode == "polling":
        if args.enable_mcp:
            print("Warning: polling mode ignores --enable-mcp.")
        if not bot_token:
